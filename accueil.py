...
class Accueil(Frame):
    ...
    def __init__(self, parent, root):
        ...
        self.grid(row=0, column=0, rowspan=100, columnspan=100)

        # message de bienvenue
        Label(self, text="PyScrabble", font=("Times", 24)).grid(row=0, columnspan=5)
        # label de la langue
        Label(self, text="Choisissez la langue du jeu:", font=("Times", 16)).grid(row=1, column=0, sticky=E,padx=self.PADX, pady=self.PADY)
        ...
        # Nombre des joueurs
        Label(self, text="Choisissez le nombre de joueurs:", font=("Times", 16)).grid(row=2, column=0, sticky=E,padx=self.PADX, pady=self.PADY)
        nb_joueurs = IntVar()
        nb_joueurs.set(2)
        ...
        Radiobutton(self, text='Règles officielles', variable=difficulte, value='difficile').grid(row=3, column=2,sticky=W)

        # Débuter la partie
        Button(self, text="Commencer une nouvelle partie",command=lambda: root.demarrer_partie(nb_joueurs.get(), langue.get(), difficulte.get())).grid(row=4, column=1,columnspan=2,sticky=NSEW,pady=self.PADY)
        Button(self, text="Charger une partie existante", command=root.charger_partie).grid(row=5, column=1,columnspan=2, sticky=NSEW,pady=self.PADY)